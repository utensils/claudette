...
fn nix_export_drops_sandbox_and_bash_builtin_vars() {
    ...
    let banned = [
        "HOME", "PATH", "SHELL", "TMPDIR", "TMP", "TEMP", "TEMPDIR",
        "PWD", "OLDPWD", "IFS", "BASH", "BASHOPTS", "SHELLOPTS", "SHLVL",
        "PPID", "PS1", "PS2", "PS3", "PS4", "TERM", "HOSTTYPE", "MACHTYPE",
        "OSTYPE", "OPTERR", "LINENO", "NIX_BUILD_CORES", "NIX_BUILD_TOP",
        "NIX_ENFORCE_PURITY", "NIX_LOG_FD", "NIX_STORE", "builder",
        "dontAddDisableDepTrack", "name", "out", "outputs", "shellHook",
        "stdenv", "system",
    ];
    for key in banned {
        assert!(
