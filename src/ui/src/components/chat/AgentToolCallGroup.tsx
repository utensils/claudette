...
export function AgentToolCallGroup({
  ...
  return (
    <div className={styles.agentToolGroup}>
      <div className={styles.agentToolGroupHeader}>
        <span className={styles.toolName} style={{ color: toolColor(activity.toolName) }}>
          {activity.toolName}
        </span>
        {summary && (
      ...
      {(activity.agentStatus || typeof activity.agentToolUseCount === "number" || activity.agentLastToolName) && (
        <div className={styles.agentToolProgress}>
          ...
          {activity.agentLastToolName && <span>latest: {activity.agentLastToolName}</span>}
        </div>
      )}
