...
ALTER TABLE conversation_checkpoints ADD COLUMN session_id TEXT
    REFERENCES chat_sessions(id) ON DELETE CASCADE;

CREATE INDEX idx_chat_messages_session
    ON chat_messages(session_id, created_at);
CREATE INDEX idx_checkpoints_session
    ON conversation_checkpoints(session_id, turn_index);

-- Backfill: create one "Main" session per workspace, carrying over the
-- workspace's current Claude session ID and turn count.
INSERT INTO chat_sessions (id, workspace_id, claude_session_id, name, name_edited,
...
-- Point existing messages at their workspace's new session.
UPDATE chat_messages SET session_id = (
    SELECT cs.id FROM chat_sessions cs
    WHERE cs.workspace_id = chat_messages.workspace_id
) WHERE session_id IS NULL;

-- Point existing checkpoints at their workspace's new session.
