...
pub enum StreamEvent {
    ...
    User {
        ...
        /// Top-level SDK user-message UUID emitted by Claude Code. When the
        /// CLI is running with `--replay-user-messages`, Claudette uses this
        /// to distinguish local stdin echoes from remote-origin messages.
        #[serde(default)]
        uuid: Option<String>,
        /// Set by the CLI when this user event replays a prompt that entered
        /// through the stream-json queue instead of normal assistant output.
    ...
    fn deserializes_replayed_remote_user_event() {
        ...
        match ev {
            StreamEvent::User {
                ...
                assert!(!is_synthetic);
                assert_eq!(uuid.as_deref(), Some("remote-msg-1"));
                assert!(is_replay);
                match message.content {
                    UserMessageContent::Text(t) => {
