//! Anthropic OAuth Usage API source (Pro/Max subscription limits).
//!
//! This is the original 839-line `src-tauri/src/usage.rs`, moved into
//! the lib crate so the Tauri side can compose it with other usage
//! sources. The behavior is identical to pre-refactor: read the OAuth
//! credentials from the platform keychain / credentials file, refresh
//! when expiring, and call `https://api.anthropic.com/api/oauth/usage`.
//!
//! Requires a Pro/Max subscription token in the keychain. API-key-only
//! installs (CLAUDE_CODE_OAUTH_TOKEN) intentionally return
...
///
/// Goes through `crate::agent::resolve_claude_path_blocking` instead of
...
// ---------------------------------------------------------------------------
// Multi-provider snapshot adapter
// ---------------------------------------------------------------------------

/// Map the Anthropic OAuth usage shape onto the unified
/// [`UsageSnapshot`](super::UsageSnapshot) the dispatcher hands to the
/// frontend. Keeps the indicator UI shape-agnostic so the same React
/// component renders Anthropic subscription buckets and local token
...
pub fn snapshot_from_usage(
    ...
    if let Some(ref b) = usage.usage.five_hour {
        buckets.push(bucket("session_5h", "Session (5h)", b, None));
    }
    if let Some(ref b) = usage.usage.seven_day {
        buckets.push(bucket("week_all", "Week (all)", b, None));
    }
    if let Some(ref b) = usage.usage.seven_day_sonnet {
        buckets.push(bucket("week_sonnet", "Week (Sonnet)", b, None));
    }
    if let Some(ref b) = usage.usage.seven_day_opus {
        buckets.push(bucket("week_opus", "Week (Opus)", b, None));
    }

    let source_label = usage
        .subscription_type
        .as_deref()
        .map(|t| {
    ...
    UsageSnapshot {
        ...
        note: Some(String::from(
            "Burn-rate weighted: the indicator surfaces whichever limit you'll hit first at your current pace.",
        )),
        fetched_at_ms,
...
mod tests {
    ...
    #[test]
    fn snapshot_maps_anthropic_buckets_to_unified_shape() {
        ...
        let usage = ClaudeCodeUsage {
            ...
            usage: UsageData {
                five_hour: Some(UsageLimit {
                    utilization: 42.0,
                    resets_at: serde_json::Value::String("2026-05-17T00:00:00Z".into()),
                }),
                seven_day: Some(UsageLimit {
                    utilization: 12.0,
                    resets_at: serde_json::Value::String("2026-05-23T00:00:00Z".into()),
                }),
        ...
        let snap = snapshot_from_usage(&usage, AgentBackendKind::Anthropic, 1_000);

        assert_eq!(snap.source_label, "Max");
        assert_eq!(snap.buckets.len(), 2);
        assert_eq!(snap.buckets[0].key, "session_5h");
        assert!((snap.buckets[0].utilization - 0.42).abs() < 0.001);
        assert!(snap.buckets[0].is_bounded);
        assert!(!snap.buckets[0].exhausted);
        assert!(!snap.experimental_disabled);
    }
}
