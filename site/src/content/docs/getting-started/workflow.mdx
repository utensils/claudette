...
3. **Switch between workspaces** by clicking them in the sidebar, opening the fuzzy finder (`⌘/Ctrl + K`), or jumping by index with `⌘/Ctrl + 1` – `⌘/Ctrl + 9` (jumps to the Nth project in group-by-repo mode, or the Nth visible workspace in group-by-status mode). The bracket shortcuts `⌘/Ctrl + Shift + [` and `⌘/Ctrl + Shift + ]` cycle the unified tab strip instead.
