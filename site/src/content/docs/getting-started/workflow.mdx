...
3. **Switch between workspaces** by clicking them in the sidebar, opening the fuzzy finder (`⌘/Ctrl + K`), or jumping to a project by index (`⌘/Ctrl + 1` – `⌘/Ctrl + 9`). The bracket shortcuts `⌘/Ctrl + Shift + [` and `⌘/Ctrl + Shift + ]` cycle the unified tab strip instead.
