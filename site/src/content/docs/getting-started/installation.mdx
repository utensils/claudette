...
- Drops the `voice` feature from the default set. The `gemm-f16` crate's ARMv8.2 inline asm requires the `fullfp16` target feature that the stock `aarch64-pc-windows-msvc` baseline doesn't enable. Opt back in via `$env:CARGO_TAURI_FEATURES` and `$env:RUSTFLAGS`.
