...
The **Settings > General > Claude Code sign-in** row triggers `claude auth login` from inside Claudette. Chat auth failures route you there, and Usage-panel credential failures reuse the same sign-in flow, so sign-in is available even when Usage Insights is disabled. There is no automatic workspace-creation login flow; if you create a workspace before signing in, the agent's first turn shows a credentials callout and you'll need to authenticate via **Settings > General** (or `claude auth login` in any terminal).
