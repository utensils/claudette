...
The **Settings > Usage** panel is the only surface that triggers `claude auth login` from inside Claudette — it does so when no usable credentials exist, when an existing token has expired and refresh fails, or on a manual click of the sign-in button. There is no automatic workspace-creation login flow; if you create a workspace before signing in, the agent's first turn fails with a credentials error and you'll need to authenticate via the Usage panel (or `claude auth login` in any terminal).
