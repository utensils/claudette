...
Claudette supports running an agent against [agent providers](/claudette/features/providers/) (Ollama, LM Studio, OpenAI, Codex) via **Settings > Models**. Each provider has its own credential expectations — see the per-provider docs. The Anthropic Pro/Max OAuth flow described above only applies when the active provider is the official `claude` CLI.
