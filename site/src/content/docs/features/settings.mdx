...
| Env provider — Always allow `.envrc` (env-direnv) | When direnv reports an `.envrc` as blocked, run `direnv allow` automatically. Bypasses direnv's per-path approval. | Off |
| Env provider — Always trust mise config (env-mise) | When mise reports config files as not trusted, run `mise trust` automatically. | Off |
