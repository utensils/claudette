...
| Language | UI language: English, Spanish, Brazilian Portuguese, Japanese, Simplified Chinese, or German. See [Internationalization](/claudette/features/internationalization/). | English |
...
| Setting | Description | Default |
|---------|-------------|---------|
| Default model | Model for new chats (Opus 5.5, Sonnet 5, Haiku 4.5, Fable 5.1; older models such as Opus 5, Fable 5 / Fable 5 1M, Opus 4.8 / 4.7 / 4.6 / 4.5 and Sonnet 4.6 / 4.6 1M remain selectable behind the picker's "More" disclosure) | — |
...
| Agent providers | Show Ollama, OpenAI API, and custom providers in Settings > Models and the chat model picker. Codex and Ollama are auto-enabled when detected unless manually disabled. | On |
...
| Agent backend cards | Enable, test, authenticate, and refresh model lists for built-in providers such as Claude Code, Codex, Ollama, and OpenAI API. | Provider-specific |
...
See [CLI Client](/claudette/features/cli-client/) for what the `claudette` command can do.

## Pinned Prompts

Manage global [pinned prompts](/claudette/features/pinned-prompts/) — reusable prompt snippets available from the composer in every workspace. Per-repo pinned prompts are configured in **Settings > Repository**.

Each prompt in the list has these controls:

| Setting | Description | Default |
|---------|-------------|---------|
| Display name | Label shown on the composer pill. Must be unique within its scope (global or per-repo). | — |
| Prompt body | Text sent when the pill is clicked. Supports the **/** slash command picker. | — |
| Auto-send when clicked | Send immediately on click instead of inserting the text into the composer for editing. | Off |
| Toolbar overrides | Tri-state (`Inherit` / `On` / `Off`) overrides for **Plan mode**, **Fast mode**, **Thinking**, and **Claude in Chrome**, applied before the prompt runs. | Inherit |
| Open in a new tab | Run the prompt in a fresh chat session named after the prompt, instead of the tab it was clicked from. | Off |
| Model | Model to select before the prompt runs. `Inherit` leaves the target session's model alone. | Inherit |
...
Each agent event (input required, plan acknowledgment, task complete, error, session start) has its own sound selector. Choose from built-in sounds or your system's sound library:
...
- A user resets or deletes `~/.claudette/data.db` but the worktree directories survive on disk.
- A dev sandbox under `/tmp/claudette-dev/new-NNNN/` (from `scripts/dev.sh --new`) was `kill -9`'d before the cleanup trap could fire.
- Workspaces were imported on another machine and the SQLite DB diverged.
...
| Show open issues & PRs in project view | Surface open issues and pull requests from the resolved SCM provider on the repo overview screen. Requires a configured SCM provider (`scm-github`, `scm-gitlab`). See [Project view — Issues & PRs](project-view-issues-prs). | Off |
...
When enabled, Claudette watches the SCM polling results for active workspaces. A new auto-fix session is created only after an observed transition to failed CI, not for a failure that was already present when polling first saw the workspace.
...
The **Shell environment** card shows env vars Claudette captured from your interactive shell init (`.zshrc`, `.bashrc`, `.zprofile`, etc.). See [Shell environment](/claudette/features/shell-environment/) for the full feature page.
