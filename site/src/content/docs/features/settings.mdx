...
| Dashboard mode (chat) | Replace the streamed agent transcript with a live activity dashboard. Each turn collapses its intermediate messages, thinking, and tool calls into a card showing counts of thoughts, questions, plans, tool calls, and tasks; only your prompt and the agent's final reply stay visible. A sticky session rollup adds turn/tool/MCP-call totals, thinking turns, tokens in/out, and top-skill and top-MCP leaderboards. Plan reviews and questions behave exactly as in the default view. Expand any turn card to reveal the full tool/thinking detail. | Off |
