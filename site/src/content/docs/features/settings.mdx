...
| Alternative Claude Code backends | Expose non-default providers in Settings > Models and the chat model picker. | Off |
| Experimental Codex | Replace the legacy `codex-subscription` gateway backend with native `codex app-server --listen stdio://` support. Requires alternative backends to be compiled in. | Off |
...
|---------|-------------|---------|
| Claudette Terminal | Show a read-only terminal tab that mirrors agent shell commands and background task output. | Off |
| Alternative Claude Code backends | Expose Ollama, LM Studio, OpenAI-compatible, and custom backends through the Claude Code harness. | Off |
