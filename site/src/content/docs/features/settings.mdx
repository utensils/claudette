...
| OpenRouter balance in usage meter | Poll OpenRouter's `/credits` endpoint and show remaining / used credits in the composer's usage popover for OpenRouter-backed sessions, including Pi OpenRouter models. Turn off to keep the meter limited to local token/cost totals. | On |
