...
| Runtime (per backend card) | Which harness Claudette spawns for this backend: **Claude CLI** (Claude Code subprocess), **ptywright Claude** (experimental hidden interactive Claude Code PTY, when compiled in), **Pi** (Claudette's bundled Pi sidecar), or **Codex app-server**. Only renders on cards whose kind has more than one valid harness — Anthropic and OpenAI cards default to Claude CLI, Ollama / LM Studio default to Pi, and Codex Native defaults to the Codex app-server. Selecting the default value clears the override so future default-policy changes pick the user back up. | Kind-specific |
