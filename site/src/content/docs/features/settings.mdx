...
| Agent backend cards | Enable, test, authenticate, and refresh model lists for built-in providers such as Ollama, LM Studio, OpenAI API, and Experimental Codex. | Provider-specific |
...
| Alternative Claude Code backends | Expose non-default providers in Settings > Models and the chat model picker. | On in supported builds |
| Experimental Codex | Replace the legacy `codex-subscription` gateway backend with native `codex app-server --listen stdio://` support. Enabling it keeps alternative backends on and seeds Codex models into the picker. | Off |
...
|---------|-------------|---------|
| Claudette Terminal | Show a read-only terminal tab that mirrors agent shell commands and background task output. | Off |
| Alternative Claude Code backends | Expose Ollama, LM Studio, OpenAI-compatible, and custom backends through the Claude Code harness. | On in supported builds |
