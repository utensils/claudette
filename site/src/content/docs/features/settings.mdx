...
| Language | UI language: English, Spanish, Brazilian Portuguese, Japanese, or Simplified Chinese. See [Internationalization](/claudette/features/internationalization/). | English |
...
| Runtime (per backend card) | Which harness Claudette spawns for this backend: **Claude CLI** (Claude Code subprocess), **ptywright Claude** (hidden interactive Claude Code PTY, when compiled in), **Pi** (Claudette's bundled Pi sidecar), or **Codex app-server**. Only renders on cards whose kind has more than one valid harness — Anthropic and OpenAI cards default to Claude CLI, Ollama / LM Studio default to Pi, and Codex Native defaults to the Codex app-server. Selecting the default value clears the override so future default-policy changes pick the user back up. | Kind-specific |
...
| Setting | Description | Default |
|---------|-------------|---------|
| Claude Code usage | Shows Claude Code subscription quota buckets and reset times by reading the official Claude Code `/usage` screen through ptywright. The composer meter uses the same source for Claude-family sessions. | On |
| Extra usage | Shows extra-usage status and opens the hosted Claude usage settings when you want to enable or manage it. | — |

**Claude Code Plugins** and **Community** are their own settings sections (visible by default in the Settings sidebar) rather than flat toggle lists — see [Community Registry & Trust](/claudette/features/community-registry-trust/) for the Community section. Claude Remote Control is reached from the chat composer's overflow menu, not Settings.
