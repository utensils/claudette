...
Per-session toggles (toolbar chip, `Shift + Tab`, `/plan`) are sticky: an explicit on/off persists for that chat session and survives app restart, session swap, and rejoining a remote session. The global default only applies the first time a session is opened with no prior choice.

Plan mode follows the active runtime. Claude-compatible sessions use Claude Code's plan approval flow; Codex Native sessions use Codex's `collaborationMode: plan` workflow. Codex may still run non-mutating exploration commands while planning, then Claudette surfaces Codex's completed plan in the same approval card before implementation begins.
...
Click **Approve** and the agent re-enters tool-use mode immediately, executing the plan it drafted. No re-prompting needed. For Codex Native, approval sends the follow-up `Implement the plan.` turn outside plan mode.

### Denying with feedback

Click **Deny** to open a feedback box. Whatever you type is sent to the agent as the next user turn, while Codex Native stays in plan mode for revisions — so this is the right path when you want a different approach, not a full restart. Examples that work well:
