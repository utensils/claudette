...
| OpenAI API / Custom OpenAI / OpenRouter | Local-aggregate tokens + OpenRouter `/credits` balance (when applicable and enabled) | Yes |
| Ollama / LM Studio / Pi | Local-aggregate tokens; Pi OpenRouter models also show OpenRouter `/credits` balance when enabled | Yes |
...
Pi sessions record token usage and provider-reported cost at the end of each turn when Pi exposes it. For OpenRouter-backed Pi models, Claudette can also poll OpenRouter's account credits endpoint with the same OpenRouter key configured for Pi; disable **OpenRouter balance in usage meter** in **Settings > Experimental** if you only want local token totals.
