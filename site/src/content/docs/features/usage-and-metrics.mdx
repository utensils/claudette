...
|---|---|---|
| Anthropic / Custom Anthropic / Codex Subscription | Anthropic OAuth Usage API (subscription quotas) | Off by default — gated by the **Claude Code Usage** toggle |
| Codex Native | Local-aggregate tokens, label from Codex `account/read` plan | Yes |
| OpenAI API / Custom OpenAI / OpenRouter | Local-aggregate tokens + OpenRouter `/auth/key` credit balance (when applicable) | Yes |
| Ollama / LM Studio / Pi | Local-aggregate tokens | Yes |
