...
Click any file in the changed-files sidebar (or use the file picker) to open it. The file opens as a tab next to your chat and diff tabs, so you can hop between agent output, the diff, and the editor without losing context.

Each file gets its own tab, and switching files inside that tab gives you a fresh undo history per file.
