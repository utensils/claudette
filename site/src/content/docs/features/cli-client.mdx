...
The `claudette` binary ships inside the desktop app bundle. Use the in-app installer in **Settings > CLI**, which symlinks the bundled binary onto your `PATH`. On Linux `.deb` installs the CLI is already at `/usr/bin/claudette` automatically. Standalone `claudette-cli-<platform>` release assets are also published per release for headless / CI consumers.
