...
5. Codex intelligence uses the same user-facing shape as Codex: `Low`, `Medium`, `High`, and `Extra High`. Claudette maps those levels to app-server `turn/start.effort` values (`low`, `medium`, `high`, `xhigh`) and seeds new threads with `config.model_reasoning_effort`. Empty or stale Codex effort values normalize to `High`.
