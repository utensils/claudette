...
For native Codex, enable **Settings > Experimental > Codex**. Codex is independent from Alternative Claude Code backends: when the Codex flag is on, **Settings > Models** shows Codex and the chat model picker includes the seeded Codex models immediately.
...
1. In **Settings > Experimental**, enable **Codex**.
2. In **Settings > Models**, find **Codex**.
...
7. Codex app-server approval requests for command execution, file changes, and permission escalation are answered over JSON-RPC. Denying one of these Codex approvals sends only a decline decision to Codex; freeform denial feedback is reserved for approval flows that can forward it.
