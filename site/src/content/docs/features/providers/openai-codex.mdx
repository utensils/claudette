...
4. The listener translates the request body into an OpenAI Responses API call (`POST /v1/responses` for OpenAI; `POST /codex/responses` for the Codex subscription path — `chatgpt.com/backend-api` already includes the API version, so the listener doesn't add `/v1/`) and translates the streaming response back into Anthropic's event format.
