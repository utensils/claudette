...
3. Click **Test** to confirm the native provider row is available.
4. Set a default model if desired.
5. Toggle **Enabled** on.
...
4. Codex notifications are adapted into Claudette chat events for assistant text, reasoning summaries, command output, token usage, turn completion, failures, and process exit.
