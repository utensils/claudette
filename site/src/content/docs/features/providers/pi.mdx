...
OpenRouter-backed Pi turns report per-turn token usage and cost in the chat footer and the composer usage meter. When **Settings > Experimental > OpenRouter balance in usage meter** is on, Claudette also polls OpenRouter's `/credits` endpoint with the OpenRouter key configured for Pi and shows remaining / used credits next to the local token totals.
