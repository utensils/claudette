...
3. Open **Settings > Models** in Claudette.
4. Find the **Pi** card.
5. Click **Refresh models** to load models from Pi's `ModelRegistry`.
6. Pick a Pi model as the default, or choose one per chat from the model picker.
...
**The sidecar cannot find Pi assets.** In a dev build, run `./scripts/stage-pi-harness-sidecar.sh` or `./scripts/stage-cli-sidecar.sh` before launching Tauri. Release builds stage this automatically.
