...
By default Claudette spawns the official `claude` CLI authenticated against Anthropic. Supported builds now keep the **Alternative Claude Code backends** runtime gate on by default, so you can also point agents at:
...
- **[OpenAI & Codex](/claudette/features/providers/openai-codex/)** — `gpt-*` models via the OpenAI API gateway, or your ChatGPT subscription through the native `codex app-server` harness.
...
1. Confirm **Settings > Experimental > Alternative Claude Code backends** is on. Supported builds enable it by default; turning it off hides all non-Anthropic providers.
2. For native Codex, toggle on **Experimental Codex**. This replaces the old `codex-subscription` gateway row with `experimental-codex`, keeps the alternative-backends gate on, and seeds Codex models into the picker.
...
| `codex-subscription` | `CodexSubscription` | `https://chatgpt.com/backend-api` | `codex login` (managed by `codex` CLI) | Legacy row shown only when **Experimental Codex** is off |
