...
On startup Claudette quietly checks for local providers. If it finds the Codex CLI, an Ollama daemon on `localhost:11434`, or an LM Studio server on `localhost:1234`, that provider is enabled automatically. Turning a local provider off in Settings records an opt-out, so Claudette will not re-enable it on the next launch. OpenAI API stays manual because it requires an API key and a remote account. Pi is not part of the startup probe — its model list is refreshed via **Settings > Models > Pi > Refresh models** (or the **Test backend** button), because the Pi SDK sidecar costs more to spawn than a localhost probe and `pi auth` may need to run first.
