...
description: Run Claudette agents against Ollama, LM Studio, OpenAI, native Codex, or Pi instead of the default Claude Code path — architecture and capability differences, with deeper setup pages for each.
---
...
- **[Pi](/claudette/features/providers/pi/)** — models/providers discovered from Pi's SDK, executed through Claudette's bundled Pi sidecar with Claudette permission mediation.
- **[ptywright Claude](/claudette/features/providers/ptywright-claude/)** — experimental feature-flagged runtime for the Anthropic card that drives interactive Claude Code in a hidden PTY instead of `claude -p`.
...
2. Keep **Agent providers** on to show Ollama, LM Studio, OpenAI API, and custom providers.
3. Keep **Codex** on to show Codex and seed Codex models into the picker.
4. Pi is always shown as a first-class harness; run `pi auth` and refresh Pi models to populate it.
5. Configure each provider you want to use (URL, secret, models).
6. Pick a default in **Settings > Models > Default model**, or override per session from the chat header.
...
|---|---|---|---|---|
| Claude Code | `Anthropic` | (uses `claude` CLI; optional ptywright runtime when compiled in) | Inherits from Claude Code | [Authentication](/claudette/features/authentication/) / [ptywright Claude](/claudette/features/providers/ptywright-claude/) |
...
| Pi | `PiSdk` | (uses bundled Pi sidecar) | `pi auth` / Pi provider config | [Pi](/claudette/features/providers/pi/) |
...
**Native and sidecar harnesses** (`CodexNative`, `PiSdk`, experimental `PtywrightClaude`) — Claudette speaks to a purpose-built subprocess or hidden interactive PTY instead of the standard print-mode Claude CLI path. Codex uses `codex app-server --listen stdio://`; Pi uses Claudette's bundled Bun sidecar embedding `@earendil-works/pi-coding-agent`; ptywright drives the real interactive `claude` TUI through a Rust PTY library and Lua classifier.

### Runtime override per backend

Some kinds support more than one harness. **Settings > Models** shows a **Runtime** select on those cards that lets you swap between the kind's default and the alternative without leaving the panel:

- **Ollama, LM Studio** — default to the **Pi** harness (more capable agent loop than proxying the Claude CLI through `ANTHROPIC_BASE_URL`). **Claude CLI** stays available as a fallback for users who prefer the original wire-format-passthrough behavior or hit a Pi limitation.
- **OpenAI API, Custom OpenAI** — default stays on **Claude CLI** (gateway translation). **Pi** is opt-in.
- **Codex Native** — default stays on the **Codex app-server**. **Pi** is opt-in (Pi's own OpenAI provider, requiring `pi auth` to be configured).
- **Anthropic** — default stays on **Claude CLI**. Experimental builds compiled with `ptywright-claude` also show **ptywright Claude**, which runs interactive Claude Code in a hidden PTY rather than `claude -p`.
- **Custom Anthropic, Codex Subscription** — locked to **Claude CLI**. Pi is **never** offered for Claude-subscription / Anthropic-OAuth flows; the resolver also enforces this at chat-send time.
...
- **Experimental ptywright Claude** — for testing the hidden interactive PTY path that avoids the Agent SDK / `claude -p` execution mode while keeping Claude Code subscription auth.
...
- **[Pi](/claudette/features/providers/pi/)** — if you want Pi's provider/model catalog while keeping Claudette's session lifecycle and approval UI.
...
- The ptywright Claude runtime is feature-flagged and currently reduced fidelity: attachments, MCP injection, host permission prompts, remote control, and native compaction still fall back to the regular Claude CLI runtime.
...
- [Ollama](/claudette/features/providers/ollama/), [LM Studio](/claudette/features/providers/lm-studio/), [OpenAI & Codex](/claudette/features/providers/openai-codex/), [Pi](/claudette/features/providers/pi/), and [ptywright Claude](/claudette/features/providers/ptywright-claude/) — per-provider setup pages
