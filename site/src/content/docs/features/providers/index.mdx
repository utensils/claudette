...
description: Run Claudette agents against Ollama, LM Studio, OpenAI, or Codex instead of the default Claude Code path — architecture and capability differences, with deeper setup pages for each.
---
...
On startup Claudette quietly checks for local providers. If it finds the Codex CLI, an Ollama daemon on `localhost:11434`, or an LM Studio server on `localhost:1234`, that provider is enabled automatically. Turning a provider off in Settings records an opt-out, so Claudette will not re-enable it on the next launch. OpenAI API stays manual because it requires an API key and a remote account.
...
- **[Codex](/claudette/features/providers/openai-codex/#codex)** — if you're already paying for ChatGPT Plus/Pro/Team and want to reuse that quota through the Codex CLI.
