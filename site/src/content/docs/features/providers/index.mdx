...
By default Claudette spawns the official `claude` CLI authenticated against Anthropic. With the **Alternative Claude Code backends** experimental flag you can also point agents at:
...
Native Codex is a separate gate: **[Experimental Codex](/claudette/features/providers/openai-codex/#experimental-codex)** uses your ChatGPT subscription through the native `codex app-server` harness and does not depend on Alternative Claude Code backends.
...
This is **experimental** because some Claude-specific features only have clean equivalents on particular backends. Claudette hides or disables unsupported controls instead of sending ignored settings.

## Enabling the feature

1. Confirm **Settings > Experimental > Alternative Claude Code backends** is on. It is off by default for new users; turning it off hides Ollama, LM Studio, OpenAI API, and custom providers.
2. For native Codex, toggle on **Experimental Codex** instead. This separate gate shows `experimental-codex` and seeds Codex models into the picker without enabling Alternative Claude Code backends.
3. Open **Settings > Models** and configure each provider you want to use (URL, secret, models).
4. Pick a default in **Settings > Models > Default backend**, or override per session from the chat header.
...
| Capability | Anthropic | Ollama | LM Studio | Gateway OpenAI | Experimental Codex |
|---|---|---|---|---|---|
| Extended thinking | ✅ | ✅ (when model supports) | ❌ | ❌ | ✅ (reasoning summaries) |
| Reasoning / effort levels | ✅ (Claude effort) | ❌ | ❌ | ❌ | ✅ (Codex reasoning effort) |
| Fast mode | ✅ | ❌ | ❌ | ❌ | ✅ |
| 1M-context auto-upgrade | ✅ | ❌ | ❌ | ❌ | ❌ |
| Tool use | ✅ | ✅ | ✅ (model-dependent) | ✅ | ✅ |
| Vision | ✅ | ✅ | ✅ (model-dependent) | ✅ | ❌ |

LM Studio is on the same direct path as Ollama, but its capability profile mirrors the gateway providers' (no extended-thinking / effort / fast / 1M toggles) — local models implement those Anthropic-specific knobs inconsistently or not at all, so the chat header hides them. Experimental Codex exposes fast mode and Codex reasoning effort through Codex app-server settings. Switching a session to a provider visibly disables or relabels the toggles that don't apply.
...
- **[Experimental Codex](/claudette/features/providers/openai-codex/#experimental-codex)** — if you're already paying for ChatGPT Plus/Pro/Team and want to reuse that quota through the native Codex CLI.
