...
On startup Claudette quietly checks for local providers. If it finds the Codex CLI, an Ollama daemon on `localhost:11434`, or an LM Studio server on `localhost:1234`, that provider is enabled automatically. Pi model discovery also refreshes through the bundled SDK sidecar when available. Turning a local provider off in Settings records an opt-out, so Claudette will not re-enable it on the next launch. OpenAI API stays manual because it requires an API key and a remote account.
...
When both provider gates are off, the built-in `Anthropic` backend and first-class Pi harness remain exposed. Workspaces on `Anthropic` run `claude` as before; Pi workspaces run through the bundled SDK sidecar.
