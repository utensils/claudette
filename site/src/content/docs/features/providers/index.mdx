...
The gateway translates Claude's request into the OpenAI Responses API (`POST /v1/responses` for OpenAI; `POST /codex/responses` for the Codex subscription path — note the absence of `/v1/`, since `https://chatgpt.com/backend-api` already includes it) and translates the streaming response back. It's an in-process tokio task; no external service.
