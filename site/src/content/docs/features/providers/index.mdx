...
Native Codex is a separate gate: **[Codex](/claudette/features/providers/openai-codex/#codex)** uses your ChatGPT subscription through the native `codex app-server` harness and does not depend on Alternative Claude Code backends.
...
2. For native Codex, toggle on **Codex** instead. This separate gate shows Codex and seeds Codex models into the picker without enabling Alternative Claude Code backends.
...
LM Studio is on the same direct path as Ollama, but its capability profile mirrors the gateway providers' (no extended-thinking / effort / fast / 1M toggles) — local models implement those Anthropic-specific knobs inconsistently or not at all, so the chat header hides them. Codex exposes fast mode and Codex reasoning effort through Codex app-server settings. Switching a session to a provider visibly disables or relabels the toggles that don't apply.
