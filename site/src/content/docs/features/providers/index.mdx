...
For gateway providers, Claudette spins up an HTTP listener on `127.0.0.1:0` (random port) per provider and per model. It mints a one-time auth token, then exports three env vars into the spawned `claude` subprocess:
