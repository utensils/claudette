...
- **[ptywright Claude](/claudette/features/providers/ptywright-claude/)** — compile-time runtime option for the Anthropic card that drives interactive Claude Code in a hidden PTY instead of `claude -p`.
...
- **Anthropic** — default stays on **Claude CLI**. Builds compiled with `ptywright-claude` also show **ptywright Claude**, which runs interactive Claude Code in a hidden PTY rather than `claude -p`.
