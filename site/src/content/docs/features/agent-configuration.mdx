...
Claudette can run agents against alternative providers — [Ollama](/claudette/features/providers/ollama/) (local, Anthropic-wire), [LM Studio](/claudette/features/providers/lm-studio/) (local, Anthropic-wire via `/v1/messages`), or [OpenAI](/claudette/features/providers/openai-codex/#openai-api) (remote, gateway-translated) — instead of the official `claude` CLI. This is controlled by the **Alternative Claude Code backends** experimental flag, which is off by default for new users. Native [Experimental Codex](/claudette/features/providers/openai-codex/#experimental-codex) has its own gate and does not depend on Alternative Claude Code backends.
...
- **Fast mode** — Anthropic and Experimental Codex.
