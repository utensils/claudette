...
While an agent turn is running, you can queue follow-up messages. Type into the chat input and hit **Enter** — each message is added to the queue popover above the composer and delivered one at a time as later user turns. Use the queue row's edit action to revise a queued message before it is delivered.
