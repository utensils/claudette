...
Claudette can run agents against alternative providers — [Ollama](/claudette/features/providers/ollama/) (local, Anthropic-wire), [LM Studio](/claudette/features/providers/lm-studio/) (local, Anthropic-wire via `/v1/messages`), [OpenAI](/claudette/features/providers/openai-codex/#openai-api) (remote, gateway-translated), or native [Codex](/claudette/features/providers/openai-codex/#codex) — instead of the official `claude` CLI. These providers are configured in **Settings > Models**; native Codex has its own gate and does not depend on Alternative Claude Code backends.
