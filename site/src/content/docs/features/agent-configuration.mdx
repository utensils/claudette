...
Only enabled flags are appended to the `claude` invocation. Disabled flags are completely omitted; they don't get a `--no-*` equivalent unless an explicit negation flag exists in the Claude CLI.
