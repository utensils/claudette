...
Claudette can run agents against alternative backends — Ollama (local), OpenAI API, or OpenAI Codex (subscription) — instead of the official `claude` CLI. This is gated by the **Alternative Claude Code backends** experimental flag.

Several chat-header toggles on this page only apply to certain backends. The capability matrix in the dedicated page shows the full picture, but the highlights:

- **Effort, fast mode, 1M-context auto-upgrade** — Anthropic only.
- **Extended thinking** — Anthropic and Ollama (when the model supports it).
- **All Claude-side toggles** — hidden on gateway backends (OpenAI / Codex), because the OpenAI Responses API has no equivalents.
