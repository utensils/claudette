...
Claudette can run an agent against an alternative backend (Codex, self-hosted, etc.) instead of the official `claude` CLI. Configure backends in **Settings > Agent Backends**. Each backend has its own credential expectations — see the backend's own documentation. The Anthropic Pro/Max OAuth flow only applies when the active backend is the official `claude` CLI.
