...
Click any project header in the sidebar (or press `⌘/Ctrl + N` for the Nth project) to open a **project-scoped view**: the same welcome card pre-targeted at that project, with the project's existing workspaces listed beneath it. Selecting a workspace from there leaves the project view; the **Back to Dashboard** arrow in the toolbar returns to the global view.
