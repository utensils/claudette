...
Click any project header in the sidebar (or press `⌘/Ctrl + 1` – `⌘/Ctrl + 9` to jump to the Nth project) to open a **project-scoped view**: a card pre-targeted at that project, with the project's existing workspaces listed beneath it. Selecting a workspace from there leaves the project view; the **Dashboard** button in the toolbar (top-left) returns to the global view.
