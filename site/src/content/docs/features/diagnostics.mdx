...
Each discovery file records the effective `CLAUDETTE_DATA_DIR` for its launch, so an instance started with `--new` or `--clone` is recognised as isolated and neither warns nor blocks database maintenance the way a genuine co-tenant does. The launcher shell that wrote the file is excluded from the scan as well — it outlives the app it starts, and its PID is the one in the filename.
