...
For databases that grew before content-addressed storage shipped, a one-time background backfill runs at app launch. It batches through legacy rows (no schema downtime — the read path falls back to the row's own `content` column for un-migrated rows), deduplicates them into the blob store, and marks completion in `app_settings.checkpoint_blob_backfill_done` so the work is a no-op on subsequent boots. Progress is logged on the `claudette::db` target.
