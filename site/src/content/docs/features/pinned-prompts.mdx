...
import composerPins from '../../../assets/screenshots/composer-pins.png';
import globalPins from '../../../assets/screenshots/global-pins.png';
import repoPins from '../../../assets/screenshots/repo-pins.png';
...
<Image src={composerPins} alt="Chat composer with a row of pinned prompt pills above the message input" loading="lazy" />
...
Beyond the toolbar toggles, a pinned prompt can decide **where** it runs and **on which model**. Both live under **Launch options** in the prompt editor and are independent of each other.

### Open in a new tab

With **Open in a new tab** enabled, clicking the pill:

1. Creates a fresh chat session in the current workspace
2. Names that tab after the prompt's display name
3. Applies the prompt's toolbar overrides and model to it
4. Switches to it and runs the prompt there
...
:::tip[The review-in-Codex workflow]
A common setup: a global **Review** prompt with the body `/code-review`, **Auto-send** on, **Open in a new tab** on, and **Model** set to a Codex model. One click on the pill while you're mid-implementation opens a separate, named review tab on a second opinion — without disturbing the session doing the work.
:::
...
<Image src={globalPins} alt="Global Pinned Prompts settings page showing several saved prompts with edit, reorder, and add controls" loading="lazy" />
...
5. Under **Toolbar overrides**, set each toggle — **Plan mode**, **Fast mode**, **Thinking**, **Claude in Chrome** — to `Inherit`, `On`, or `Off` (see [Toolbar Overrides](#toolbar-overrides) above; leave them on `Inherit` if the prompt doesn't care)
6. Under **Launch options**, optionally enable **Open in a new tab** and pick a **Model** (see [Launch Options](#launch-options) above)
7. Click **Save**
...
<Image src={repoPins} alt="Repository settings page with a Pinned Prompts section and an Inherited from Global list below it" loading="lazy" />
