...
Permission modes control which Claude Code tools Claudette pre-approves for the session. `readonly` allows read/search/web tools plus the `Agent` subagent tool, so exploratory fan-out can still run without granting edit or shell access. `standard` adds file editing tools. `full` uses Claude Code's bypass-permissions mode for the session.
