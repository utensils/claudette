...
Exposes Ollama (local), LM Studio (local), and the OpenAI API in addition to the default Claude Code path. This gate is off by default for new users. Codex is intentionally separate: enable **Experimental Codex** to use the native `codex app-server` harness. Custom Anthropic / OpenAI endpoint kinds exist in the data model but the GUI doesn't yet have an "Add backend" flow.
