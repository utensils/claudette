...
Exposes Ollama (local), LM Studio (local), the OpenAI API, and Codex in addition to the default Claude Code path. With **Experimental Codex** enabled, Codex uses the native `codex app-server` harness; with that gate off, the legacy Codex subscription gateway row remains available. Custom Anthropic / OpenAI endpoint kinds exist in the data model but the GUI doesn't yet have an "Add backend" flow.
