...
If transcription returns no text or surfaces a "Windows speech recognizer is not installed" error, install the speech recognizer language pack: **Settings → Time & language → Speech → Add a voice → English (United States)** (or your preferred locale's recognizer). Without an installed recognizer the engine has nothing to match against.

If transcription fails for another reason, the toolbar shows a short message and the underlying .NET exception chain is written to the daily diagnostics log. Open it via **Settings → Diagnostics → Open log directory** for the full context.
