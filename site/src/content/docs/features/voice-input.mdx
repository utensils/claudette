---
title: Voice Input
description: Speak prompts directly into the chat composer using Apple Speech, the Windows Speech API (SAPI 5.4), on-device Distil-Whisper, or the Web Speech API.
---
...
| **Distil-Whisper** | macOS, Linux, Windows | Fully on-device | Uses the [`distil-whisper/distil-large-v3`](https://huggingface.co/distil-whisper/distil-large-v3) model (~1.5 GB), downloaded once from Hugging Face on first use and cached locally. All subsequent transcription is offline. Metal GPU acceleration on macOS keeps latency low; on CPU (Linux + Windows, including ARM64) inference is materially slower — for short dictation prefer Windows SAPI / Apple Speech, and the timeout ceiling is set to 5 min on CPU vs 90 s on macOS to give long clips room. |
