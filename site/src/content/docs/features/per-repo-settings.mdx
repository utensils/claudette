...
The **Environment** section in each repo's settings exposes a per-repo override for every env-provider plugin (direnv, mise, dotenv, Nix devshell). Each plugin's manifest-declared settings — currently `timeout_seconds` for all four, plus `auto_allow` (direnv) / `auto_trust` (mise) — show up as a form. Setting a value here only applies to workspaces in this repo; cleared fields fall back to the global default in **Settings → Plugins**.
